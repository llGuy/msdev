...
private:
	...
	typedef std::string(CalcParser::*func_t)(const std::string&);
	//array of pointer to the functions
	//add(),sub() etc..
	func_t _basicArithmetic[5];		//add, sub, mul, div, mod
	//returns an array of two numbers, used to work out
	//the answer to an operation
	unsigned int* GenerateStrInt(const std::string &exp) {
	...
	std::string Add(const std::string &exp) {
	...
	std::string Sub(const std::string &exp) {
		unsigned int* _arrNum = GenerateStrInt(exp);
	...
	std::string Mul(const std::string &exp) {
		unsigned int* _arrNum = GenerateStrInt(exp);
	...
	std::string Div(const std::string &exp) {
		unsigned int* _arrNum = GenerateStrInt(exp);
	...
	std::string Mod(const std::string &exp) {
		unsigned int* _arrNum = GenerateStrInt(exp);
		//the modulo takes place here
		unsigned int _resultInt = _arrNum[0] % _arrNum[1];
		delete _arrNum;
		return ConvertIntToString(_resultInt);
	}
	...
	std::string CheckForVar(const std::string &exp) {
		size_t _index = 0;
		...
		while(_expression[_index] != '\0') {
			...
			if(_t != '('&&_t != ')') {
				...
				//to mean something
				if(((int)_t < 48 || (int)_t > 57) && (_t!= '+'&&_t != '-'&&_t != '*'&&_t != '/'&&_t != '%'&&_t != ' ')) {
...
protected:
	...
	inline std::string ParseStrBefore(unsigned int posOfEnd,const std::string &exp) {
		std::string _result = "";
		unsigned int _index = 0;
	...
	inline std::string ParseStrAfter(unsigned int posOfStart,const std::string &exp,bool var = false) {
		std::string _result = "";
	...
	inline unsigned int CalculateWords(const std::string &exp) {
	...
	std::string Reduce(std::string &currExp,std::string oriStrBeforeP,std::string oriStrAfterP,unsigned int numOfBrackets = 0) {
		...
		if(CalculateWords(currExp) > 1) {
			...
			for(unsigned int _indexVec = 0; _indexVec < _vecWords._size; _indexVec++) {
				for(unsigned int _op = 0; _op < 5; _op++) {
					if(_vecWords[_indexVec][0] == OPERATIONS[_op] && (OPERATIONS[_op] == '*' || OPERATIONS[_op] == '/' || OPERATIONS[_op] == '%')) {
		...
		MUL_DIV:if(_result != "") {
			...
			ADD_SUB:std::string _newCurrExp = "";
				...
				else _newCurrExp = RenewExp(_vecWords,_indexOfOp,_result);
				//calls the function again
...
public:
	...
	explicit CalcParser(void) {
		...
		_basicArithmetic[4] = &CalcParser::Mod;
	}
	~CalcParser(void) {}
