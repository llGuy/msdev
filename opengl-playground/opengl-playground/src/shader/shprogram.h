...
public:
	explicit SHProgram(const std::string& vshDir,
		const std::string& fshDir, const std::string& gshDir, const bool&& hasGeometry = true)
		: m_vsh(GL_VERTEX_SHADER, vshDir),
		m_fsh(GL_FRAGMENT_SHADER, fshDir), m_hasGeometry(hasGeometry)
	{
		if (m_hasGeometry)
		{
...
public:
	...
	void Link(const std::vector<const char*>& locs)
	{
			...
			&& CheckShaderStatus(m_fsh.ShaderID()))
		{
			if (m_hasGeometry)
			{
				if (CheckShaderStatus(m_gsh.ShaderID()))
				{
