...
const size_t g_0ASCII = 48;
const size_t g_9ASCII = 57;
...
std::string G_BuildString(const char p_endCh,size_t p_szetIter,const std::string& p_strOri) {
...
inline size_t G_GetQuantOfDigits(size_t p_uint) {
...
inline size_t G_Power(int p_base,int p_exp) {
	if(p_exp == 0) return 1;
...
inline size_t G_ExtractValueOfStrUInt(const std::string& p_strUInt) {
...
inline std::string G_ConvertUIntToStr(size_t p_uint) {
...
inline size_t G_CalculateQuantOfWords(const std::string& p_str) {
	size_t l_index = 0;
...
inline bool G_CheckForVar(const std::string& p_RSVStr) {
...
inline type G_FetchTypeOfVar(const std::string& p_varStr) {
...
inline type G_FetchTypeOfConst(const std::string& p_RSVStr) {
