...
#include "../misc/Misc.h"
#include "../cline/IncludeCLine.h"

struct Clock {
	static std::chrono::steady_clock::time_point m_start;
	static std::chrono::steady_clock::time_point m_end;
	static void M_ClockStart(void) { Clock::m_start = std::chrono::steady_clock::now(); };
	static void M_ClockEnd(void) { Clock::m_end = std::chrono::steady_clock::now(); };
	static double M_GetDifference() { 
		std::chrono::steady_clock::duration l_timeElapsed = m_end - m_start;
		double l_duration = (double)std::chrono::duration_cast<std::chrono::microseconds>(l_timeElapsed).count() / 1000000;
		return l_duration;
	};
};

std::chrono::steady_clock::time_point Clock::m_start;
std::chrono::steady_clock::time_point Clock::m_end;
...
public:
	void M_Execute(void) {
		...
		Clock::M_ClockEnd();
		std::cout << "\n\nTIME ELAPSED " << Clock::M_GetDifference() << "s\n\n" << std::endl;
	}
private:
	...
	public:
		void M_InitLineCompilation(void) {
			...
			m_exeMap["else"] = [&](size_t &p_index,Vector<std::string> &p_vecLineStr,Vector<CLine::LineOfCode*> &p_vecLine)->void {
				Vector<std::string> l_body(Methods::M_BuildBody(p_vecLineStr,p_index));
				CLine::LineOfCode* l_else = new CLine::LineOfCode_Else(l_body);
				p_vecLine.M_PushBack(l_else);
				Methods::M_Parse(*l_else->M_GetBodyStringLines(),
					*(l_else->M_GetBodyLineObj()));
			};
